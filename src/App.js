...
import './App.css';
import Message from './context/message/getMessage';
import ProjectList from './context/ProjectList';

import { DeleteProject, GetProjects, ProcessCount } from './features/projects/ProjectsSlice';
import { GetMessages } from './features/messages/messageSlice';

function App() {
  const [isGrid, setIsGrid] = useState(true);

  const getProjects = useSelector(GetProjects);
  const getProcessCount = useSelector(ProcessCount);
  // const delProject = useSelector(DeleteProject);
  const getMessages = useSelector(GetMessages);
  ...
  <div className="app-header-left">
    ...
    <div className="app-header-right">
      <button className={`mode-switch${isLight ? '' : ' active'}`} title="Switch Theme" onClick={() => setIsLight(!isLight)}>
  ...
  <div className="app-content">
    ...
    <div className="projects-section">
      ...
      <div className="projects-section-line">
        <div className="projects-status">
          <div className="item-status">
            <span className="status-number">{getProcessCount}</span>
            <span className="status-type">In Progress</span>
          </div>
          <div className="item-status">
            <span className="status-number">{getProjects.length}</span>
            <span className="status-type">Total Projects</span>
          </div>
        </div>
        <div className="view-actions">
          <button className={isGrid ? 'view-btn list-view' : "view-btn list-view active"} title="List View" onClick={() => {
              setIsGrid(false);
          }}>
          ...
          <button className={isGrid ? 'view-btn grid-view active' : "view-btn grid-view"} title="Grid View" onClick={() => {
              setIsGrid(true);
          }}>
      ...
      <div className={isGrid ? 'project-boxes jsGridView' : 'project-boxes jsListView'}>
          <ProjectList 
